...
typedef struct {
    ...
    XCanny_edge_detection_uio_map maps[ MAX_UIO_MAPS ];
} XCanny_edge_detection_uio_info;

/***************** Variable Definitions **************************************/
static XCanny_edge_detection_uio_info uio_info;
...
static int uio_info_read_name(XCanny_edge_detection_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCanny_edge_detection_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCanny_edge_detection_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCanny_edge_detection_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCanny_edge_detection_Initialize(XCanny_edge_detection *InstancePtr, const char* InstanceName) {
	XCanny_edge_detection_uio_info *InfoPtr = &uio_info;
